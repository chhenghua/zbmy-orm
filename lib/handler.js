...
var mysqlResultReturn = function(_callback){
...
var mysqlErrReturn = function(_callback){
    return function(err){
        _callback(err, null);
        return null;
    }
    ...
    mysql: function(_method){
        switch(_method){
            ...
            case "getSequelize":
                return function(_model, _args, _callback){
                    return {
                        sequelize: self.dbs[_model.sqlType][_model.dialect].sequelize,
                        Sequelize: self.dbs[_model.sqlType][_model.dialect].Sequelize
                    };
                };
                break;
            case "transaction":
                return function(_model, _args, _callback){
                    var sequelize = self.dbs[_model.sqlType][_model.dialect].sequelize;
                    var Sequelize = self.dbs[_model.sqlType][_model.dialect].Sequelize;
                    // 显式提交
                    ...
                    // return sequelize.transaction({
                    //     isolationLevel: _args[0].isolationLevel || Sequelize.Transaction.ISOLATION_LEVELS.REPEATABLE_READ,
                    //     deferrable: _args[0].deferrable || Sequelize.Deferrable.NOT,
                    //     autocommit: _args[0].autocommit || false
                    // },function(t){
                    //     return _callback(t);
            ...
            case "invokeSQL":
                return function(_model, _args, _callback){
                    var sequelize = self.dbs[_model.sqlType][_model.dialect].sequelize;
                    var Sequelize = self.dbs[_model.sqlType][_model.dialect].Sequelize;
                    return sequelize.query.apply(sequelize, _args)
                        ...
                        //     });
                        // })
                        .then(mysqlResultReturn(_callback))
                        .catch(mysqlErrReturn(_callback));
                };
                break;
            case "findOrCreate":
                return function(_model, _args, _callback){
                    return _model.findOrCreate.apply(_model, _args)
            ...
            case "decrement":
                return function(_model, _args, _callback){
                    return _model.findOne(_args[0]).then(function(model){
                        if(!_.isNull(model) && !_.isEmpty(model)){
                            // delete _args[0]
                            Array.prototype.shift.call(_args);
                            model[_method].apply(model, _args)
                                .then(mysqlResultReturn(_callback))
                                .catch(mysqlErrReturn(_callback))
                        }
                        return null;
            ...
            default :
                return function(_model, _args, _callback){
                    if(/w_/.test(_method)){
                        _method = _method.substr(2)
                    }
                    return _model[_method].apply(_model, _args)
                        .then(mysqlResultReturn(_callback))
                        .catch(mysqlErrReturn(_callback));
                };
                break;
...
Handler.oracle = Handler.mysql


var HandlerFactory = function(_modelName, _method, _arguments){
    var model = null, result = null, callback = _.isFunction(_arguments[_arguments.length-1])?_arguments[_arguments.length-1]:function(_r){
        return _r;
    ...
    var checkModelFunc = function(_sqlType){
        ...
        if(!_.isEmpty(cacheModel) && _.isObject(cacheModel)){
            ...
            if(_.isFunction(cacheHandler)){
                result = cacheHandler(cacheModel, _arguments, callback);
            } else {
                handlerMapping[dbName+"_"+_method] = Handler[dbName](_method);
                result = handlerMapping[dbName+"_"+_method](cacheModel, _arguments, callback);
            }
        }else{
            ...
            _.mapKeys(dbs, function(value, key){
                ...
                if(!_.isEmpty(model) && _.isObject(model)) {
                    ...
                    // console.log(handlerMapping[key + "_" + _method](model, _arguments, callback));
                    result = handlerMapping[key + "_" + _method](model, _arguments, callback);
                } else {
                    console.warn("The model named "+_modelName+" does not exist!!!");
                }
            });
        }
        // return model;
